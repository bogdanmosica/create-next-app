...
export async function createSaaSLibStructure(projectPath) {
    ...
    // Create auth files
    await fs.writeFile(path.join(libPath, "auth", "middleware.ts"), authMiddlewareTemplate);
    await fs.writeFile(path.join(libPath, "auth", "session.ts"), authSessionTemplate);
    // Create db files
    await fs.writeFile(path.join(libPath, "db", "drizzle.ts"), dbDrizzleTemplate);
    await fs.writeFile(path.join(libPath, "db", "queries.ts"), dbQueriesTemplate);
    await fs.writeFile(path.join(libPath, "db", "schema.ts"), dbSchemaTemplate);
    await fs.writeFile(path.join(libPath, "db", "seed.ts"), dbSeedTemplate);
    await fs.writeFile(path.join(libPath, "db", "setup.ts"), dbSetupTemplate);
    // Create payments files
    await fs.writeFile(path.join(libPath, "payments", "actions.ts"), paymentsActionsTemplate);
    await fs.writeFile(path.join(libPath, "payments", "stripe.ts"), paymentsStripeTemplate);
    // Create migrations directory .gitkeep
    await fs.writeFile(path.join(libPath, "db", "migrations", ".gitkeep"), "");
}
export async function updatePackageJsonForSaaS(projectPath) {
...
//# sourceMappingURL=saas-lib-creators.js.map