...
export async function createActionsReadme(projectPath) {
    const content = `## Example of action 
...
\`\`\``;
    await fs.writeFile(path.join(projectPath, "actions", "README.md"), content);
}
export async function createComponentsReadme(projectPath) {
    const content = `## Example use of a Shadcn component:
...
> **IMPORTANT** if a constant like \`ObjectType\` in this example is used across all the other components it should be saved under \`lib/constants/object-type.ts\` in our case, that goes for any other constants.`;
    await fs.writeFile(path.join(projectPath, "components", "README.md"), content);
}
export async function createLibReadme(projectPath) {
    const content = `## Lib folder structure
...
- \`lib/validations.ts\` - Validation schemas (Zod recommended)`;
    await fs.writeFile(path.join(projectPath, "lib", "README.md"), content);
}
export async function createLibDbReadme(projectPath) {
    const content = `> The folder structure and code split I expect.
📂 db
    └ 📂 schema
      ├ 📜 users.sql.ts
      ├ 📜 common.sql.ts
...
\`\`\``;
    await fs.writeFile(path.join(projectPath, "lib", "db", "README.md"), content);
}
